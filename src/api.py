...
# ----- master parquet with auto-reload --------------------------------------
# The updater bot rewrites this file daily. We previously read it once at
# import time; uvicorn workers then served stale data until restart. Now we
# check the file's mtime on every request and reload if it has changed.
...
def _ensure_fresh():
    """Cheap mtime check; called from every data endpoint."""
    _reload_master()


@app.post("/api/admin/reload")
def admin_reload(token: str = Query("")):
    """Manual reload trigger. Guarded by the same secret as JWT signing."""
    expected = os.getenv("ARKAFON_ADMIN_TOKEN")
    if not expected or token != expected:
        raise HTTPException(status_code=403, detail="forbidden")
    reloaded = _reload_master(force=True)
    return {"reloaded": reloaded, "rows": int(len(df))}
...
def get_macro_kpi(start: str = "", end: str = "", categories: str = ""):
    ...
    if f_df.empty:
        return {"total_net_inflow": 0, "unique_funds": 0,
                "top_fund": {"code": "-", "name": "-"}}
    total_inflow = float(f_df["net_giris_tl"].sum())
    unique_funds = int(f_df["FONKODU"].nunique())
    top = f_df.groupby(["FONKODU", "FONUNVAN"])["net_giris_tl"].sum()
    if top.empty:
        top_fund = {"code": "-", "name": "-"}
    else:
        code, name = top.idxmax()
        top_fund = {"code": code, "name": name}
    return {"total_net_inflow": total_inflow,
...
def get_macro_trend(start: str = "", end: str = "", categories: str = ""):
    f_df = get_filtered_df(start, end, categories)
    if f_df.empty:
        return {"series": [], "window_relative": True}
    ...
    # Window-relative cumulative (resets at the start of the filter window).
    trend["kumulatif_giris"] = trend["net_giris_tl"].cumsum()
    return {"series": trend.to_dict(orient="records"),
...
def get_macro_top10(start: str = "", end: str = "", categories: str = ""):
    ...
    return top.rename(columns={"FONKODU": "code", "FONUNVAN": "name",
                                "net_giris_tl": "netInflow"}).to_dict(orient="records")
...
def get_macro_universe(start: str = "", end: str = "", categories: str = ""):
    ...
    return grouped.rename(columns={"FONKODU": "code", "FONUNVAN": "name",
                                    "ana_kategori": "category"}).to_dict(orient="records")


# ----- fund list (paginated + searchable) -----------------------------------
...
def get_fund_list(
    ...
    page_df = funds.iloc[start:end]
    return {
        "items": page_df.rename(columns={"FONKODU": "code", "FONUNVAN": "name"}).to_dict(orient="records"),
        "total": total,
        "page": page,
        "page_size": page_size,
    }
...
def get_fund_details(fund_code: str, start: str = "", end: str = ""):
    ...
    return {
        ...
        "chart": chart_data.rename(columns={"tarih": "date", "FIYAT": "price",
                                              "net_giris_tl": "flow"}).to_dict(orient="records")
    }
...
def get_macro_flow_sankey(start: str = "", end: str = "", categories: str = ""):
    ...
    nodes = [{"name": name, "aum": aum_dict.get(name, 0)} for name in all_names]
    name_to_idx = {name: i for i, name in enumerate(all_names)}
    links = []
    for _, row in flow_df.iterrows():
...
def get_portfolio_chart(funds: str = "", start: str = "", end: str = ""):
    ...
    try:
        ...
        for date, row in index_df.iterrows():
            tefas_val = row.mean()
            valid_funds = [f for f in fund_list if f in row.index and not pd.isna(row[f])]
